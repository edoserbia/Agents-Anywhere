...
**每次客户端改动都必须提升版本号**，不能只改代码不升版本：已发布的安装包不会因为源码变化而更新，用户也无法判断自己装的是哪一版。默认做法是补丁位加一（当前 `2.0.3` → 下一版 `2.0.4`）。

### 发版必须同步下载页

发版不只是打包。远端下载页（`http://124.220.147.199:4001`）是用户获取安装包的唯一入口，**每次发版都必须一起更新**，否则用户下载到的仍是旧版本：

1. 提升上述五个位置（含 `AppConfig.UPDATE_DOWNLOAD_URL`）的版本号。
2. 重新构建 APK 与 DMG。
3. 上传到远端 `/opt/aa-downloads/`，并逐字节校验哈希。
4. 更新 `SHA256SUMS.txt`。
5. 更新下载页的版本号与文件名。
6. 部署服务端镜像，使 `/api/v2/health` 返回新版本。

`devtools/release.sh <版本号>` 自动完成 1–5 步：

```
devtools/release.sh 2.0.4
```

脚本会强制校验上传后哈希一致、校验和可验证、且下载页确实显示了新版本，任何一步不满足都会失败退出。服务端部署与提交需要另行执行（见脚本末尾提示），因为它们的时机由部署环境决定。
